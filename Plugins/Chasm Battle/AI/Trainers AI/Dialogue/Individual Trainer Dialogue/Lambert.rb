PokeBattle_AI::TrainerSendsOutPokemonDialogue.add(:LAMBERT,
  proc { |_policy, battler, trainer_speaking, dialogue_array|
      if battler.battle.pbAbleCount(battler.index) == battler.battle.sideSizes[1] && !trainer_speaking.policyStates[:LastPokemonComment]
          dialogue_array.push(_INTL("Got me on the ropes, huh? Well it ain't over till it's over."))
...
PokeBattle_AI::TrainerPokemonTookMoveDamageDialogue.add(:LAMBERT,
  proc { |_policy, _dealer, taker, _move, trainer_speaking, dialogue_array|
      ...
      next dialogue_array unless taker.hasActiveAbility?(:ILLUSION)
      if taker.damageState.typeMod < Effectiveness::NORMAL_EFFECTIVE
          dialogue_array.push(_INTL("Hah. Oldest trick in the book."))
      elsif taker.damageState.typeMod > Effectiveness::NORMAL_EFFECTIVE
          dialogue_array.push(_INTL("Saw through my trick, did ya?"))
      end
...
PokeBattle_AI::TrainerPokemonImmuneDialogue.add(:LAMBERT,
  proc { |_policy, _attacker, target, _isImmunityAbility, trainer_speaking, dialogue_array|
      ...
      dialogue_array.push(_INTL("Mystified? I've got plenty of tricks up my sleeve."))
      trainer_speaking.policyStates[:IllusionComment] = true
      next dialogue_array
  }
)
