...
class SpriteWindow_DebugVariables < Window_DrawableCommand
    ...
    def drawItem(index, _count, rect)
        ...
        shadowtext(rect.x + idWidth, rect.y, nameWidth, rect.height, name, 0, codeswitch ? 1 : 0)
...
def pbDebugDayCare
    ...
    loop do
        if refresh
            ...
            textpos = []
            for i in 0...2
                textpos.push([_INTL("Pokémon {1}", i + 1), Graphics.width / 4 + i * Graphics.width / 2, 2, 2, base,
                              shadow,])
            end
            for i in 0...pbDayCareDeposited
                next unless $PokemonGlobal.daycare[i][0]
                y = 34
        ...
        if Input.trigger?(Input::BACK)
            ...
            when 3   # Generate/discard egg
                if pbEggGenerated?
                    ...
                    refresh = true
                elsif pbDayCareDeposited != 2 || pbDayCareGetCompat == 0
                    pbPlayBuzzerSE
                else
                    pbPlayDecisionSE
...
class SpriteWindow_DebugRoamers < Window_DrawableCommand
    ...
    def drawItem(index, _count, rect)
        ...
        if index == itemCount - 2
            # Advance roaming
            shadowtext(_INTL("[All roam to new locations]"), rect.x, rect.y, nameWidth, rect.height)
        elsif index == itemCount - 1
            # Advance roaming
            shadowtext(_INTL("[Clear all current roamer locations]"), rect.x, rect.y, nameWidth, rect.height)
        else
            ...
            shadowtext(name, rect.x, rect.y, nameWidth, rect.height)
            shadowtext(status, rect.x + nameWidth, rect.y, statusWidth, rect.height, 1, statuscolor)
        end
    end
end
    ...
    loop do
        ...
        if Input.trigger?(Input::ACTION) && cmdwindow.index < cmdwindow.roamerCount &&
           ...
           $PokemonGlobal.roamPokemon[cmdwindow.index] != true
            ...
            elsif Settings::ROAMING_SPECIES.length == 0 # Clear all roaming locations
                pbPlayBuzzerSE
            else
                pbPlayDecisionSE
...
def pbImportAllAnimations
    ...
    if animationFolders.length == 0
        ...
        for folder in animationFolders
            ...
            %w[wav ogg mid wma mp3].each do |ext|
                upext = ext.upcase
                audios.concat(files.find_all { |f| f[f.length - 3, 3] == ext })
                audios.concat(files.find_all { |f| f[f.length - 3, 3] == upext })
            end
            for audio in audios
                pbSafeCopyFile(audio, RTP.getAudioPath("Audio/SE/Anim/" + File.basename(audio)),
      "Audio/SE/Anim/" + File.basename(audio))
            end
            images = []
            %w[png gif].each do |ext| # jpg jpeg bmp
                upext = ext.upcase
                images.concat(files.find_all { |f| f[f.length - 3, 3] == ext })
                images.concat(files.find_all { |f| f[f.length - 3, 3] == upext })
            end
            for image in images
                pbSafeCopyFile(image, RTP.getImagePath("Graphics/Animations/" + File.basename(image)),
      "Graphics/Animations/" + File.basename(image))
            end
            Dir.glob(folder + "/*.anm") do |f|
                textdata = begin
                    loadBase64Anim(IO.read(f))
                rescue StandardError
                    nil
                end
                if textdata && textdata.is_a?(PBAnimation)
                    ...
                    if textdata.graphic && textdata.graphic != "" && (!safeExists?(folder + "/" + textdata.graphic) &&
                           !FileTest.image_exist?("Graphics/Animations/" + textdata.graphic))
                        textdata.graphic = ""
                        missingFiles.push(textdata.graphic)
                    end
                    for timing in textdata.timing
                        next unless timing.name && timing.name != "" && (!safeExists?(folder + "/" + timing.name) &&
    ...
    def pbConfirm(text)
        ...
        using(cmdwindow = Window_CommandPokemon.new([_INTL("Yes"), _INTL("No")])) do
            ...
            loop do
                Graphics.update
                Input.update
                cmdwindow.visible = true unless @messageBox.busy?
                cmdwindow.update
                pbUpdate
                unless @messageBox.busy?
                    if Input.trigger?(Input::BACK)
                        ret = false
                        break
