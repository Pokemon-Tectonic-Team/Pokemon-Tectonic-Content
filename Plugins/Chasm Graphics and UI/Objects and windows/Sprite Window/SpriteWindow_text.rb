...
class Window_UnformattedTextPokemon < SpriteWindow_Base
    ...
    def initialize(text = "")
        ...
        colors = getDefaultTextColors(windowskin)
        @baseColor = colors[0]
    ...
    def resizeToFitInternal(text, maxwidth) # maxwidth is maximum acceptable window width
        ...
        getLineBrokenChunks(contents, text,
           cwidth - borderX - SpriteWindow_Base::TEXTPADDING, dims, true)
        return dims
    end
    ...
    def resizeToFit(text, maxwidth = -1) # maxwidth is maximum acceptable window width
        ...
        self.width = dims[0] + borderX + SpriteWindow_Base::TEXTPADDING
        self.height = dims[1] + borderY
        refresh
    end
    ...
    def refresh
        self.contents = pbDoEnsureBitmap(contents, width - borderX,
           height - borderY)
        contents.clear
        drawTextEx(contents, 0, 4, contents.width, 0,
...
class Window_AdvancedTextPokemon < SpriteWindow_Base
    ...
    def width=(value)
        super
        self.text = text unless @starting
    end
    ...
    def resizeToFit2(text, maxwidth, maxheight)
        ...
        oldstarting = @starting
        @starting = true
        self.width  = [dims[0] + borderX + SpriteWindow_Base::TEXTPADDING, maxwidth].min
        self.height = [dims[1] + borderY, maxheight].min
        @starting = oldstarting
        redrawText
    end
    ...
    def resizeToFitInternal(text, maxwidth)
        dims = [0, 0]
        cwidth = (maxwidth < 0) ? Graphics.width : maxwidth
        chars = getFormattedTextForDims(contents, 0, 0,
           cwidth - borderX - 2 - 6, -1, text, @lineHeight, true)
        for ch in chars
    ...
    def setText(value)
        ...
        height = 1
        numlines = 0
        visiblelines = (self.height - borderY) / 32
        if value.length == 0
            @fmtchars     = []
            @bitmapwidth  = width
            @bitmapheight = height
            ...
            if @letterbyletter
                @fmtchars = []
                fmt = getFormattedText(contents, 0, 0,
                   self.width - borderX - SpriteWindow_Base::TEXTPADDING, -1,
                   shadowctag(@baseColor, @shadowColor) + value, 32, true)
                @oldfont = contents.font.clone
                for ch in fmt
                    chx = ch[1] + ch[3]
                    chy = ch[2] + ch[4]
                ...
                @fmtchars = getFormattedText(contents, 0, 0,
                   self.width - borderX - SpriteWindow_Base::TEXTPADDING, -1,
                   shadowctag(@baseColor, @shadowColor) + value, 32, true)
                @oldfont = contents.font.clone
                for ch in @fmtchars
    ...
    def skipAhead
        ...
        resume
        visiblelines = (height - borderY) / @lineHeight
        loop do
            curcharSkip(999)
    ...
    def moveCursor
        ...
        case cursor
        when 0   # End of text
            @pausesprite.x = x + startX + @endOfText.x + @endOfText.width - 2
            @pausesprite.y = y + startY + @endOfText.y - @scrollY
        when 1   # Lower right
            pauseWidth  = @pausesprite.bitmap ? @pausesprite.framewidth : 16
            pauseHeight = @pausesprite.bitmap ? @pausesprite.frameheight : 16
            @pausesprite.x = x + width - (20 * 2) + (pauseWidth / 2)
            @pausesprite.y = y + height - (30 * 2) + (pauseHeight / 2)
        when 2   # Lower middle
            pauseWidth  = @pausesprite.bitmap ? @pausesprite.framewidth : 16
            pauseHeight = @pausesprite.bitmap ? @pausesprite.frameheight : 16
            @pausesprite.x = x + (width / 2) - (pauseWidth / 2)
            @pausesprite.y = y + height - (18 * 2) + (pauseHeight / 2)
        end
    end
    ...
    def refresh
        ...
        numchars = [@curchar, @numtextchars].min if letterbyletter
        return if busy? && @drawncurchar == @curchar && @scrollstate == 0
        if !letterbyletter || !oldcontents.equal?(contents)
            @drawncurchar = -1
            @needclear = true
        end
        if @needclear
            contents.font = @oldfont if @oldfont
            contents.clear
            @needclear = false
        end
        if @nodraw
        ...
        for i in @drawncurchar + 1..numchars
            ...
            unless letterbyletter
                next if @fmtchars[i][1] >= maxX
                next if @fmtchars[i][2] >= maxY
            end
            drawSingleFormattedChar(contents, @fmtchars[i])
            @lastDrawnChar = i
        end
        if !letterbyletter && @oldfont
            # all characters were drawn, reset old font
            contents.font = @oldfont
        end
    ...
    def update
        ...
        if busy?
            refresh unless @frameskipChanged
            updateInternal
...
class Window_InputNumberPokemon < SpriteWindow_Base
    ...
    def initialize(digits_max, min = nil, max = nil)
        ...
        @negative = false
        super(0, 0, 32, 32)
        self.width = digits_max * 24 + 8 + borderX
        self.height = 32 + borderY
        colors = getDefaultTextColors(windowskin)
        @baseColor = colors[0]
        @shadowColor = colors[1]
        @index = digits_max - 1
    ...
    def number=(value)
        value = 0 unless value.is_a?(Numeric)
        if @sign
            @negative = (value < 0)
    ...
    def sign=(value)
        @sign = value
        self.width = @digits_max * 24 + 8 + borderX + (@sign ? 24 : 0)
        @index = (@digits_max - 1) + (@sign ? 1 : 0)
        refresh
    end

    def refresh
        self.contents = pbDoEnsureBitmap(contents,
           width - borderX, height - borderY)
        pbSetSystemFont(contents)
        contents.clear
        s = format("%0*d", @digits_max, @number.abs)
        if @sign
    ...
    def textHelper(x, y, text, i)
        ...
        pbDrawShadowText(contents, x + (12 - textwidth / 2), y, textwidth + 4, 32, text, @baseColor, @shadowColor)
        if @index == i && @active && @frame / 15 == 0
            contents.fill_rect(x + (12 - textwidth / 2), y + 30, textwidth, 2, @baseColor)
        end
    end
end
...
class SpriteWindow_Selectable < SpriteWindow_Base
    ...
    def itemRect(item)
        if item < 0 || item >= @item_max || item < top_item ||
           item > top_item + page_item_max
            return Rect.new(0, 0, 0, 0)
        else
            cursor_width = (width - borderX - (@column_max - 1) * @column_spacing) / @column_max
    ...
    def update
        super
        if active && @item_max > 0 && @index >= 0 && !@ignore_input
            if Input.repeat?(Input::UP)
                if @index >= @column_max ||
    ...
    def priv_page_row_max
        return (height - borderY) / @row_height
    end

    def priv_page_item_max
        return (height - borderY) / @row_height * @column_max
    end
    ...
    def priv_update_cursor_rect(force = false)
        ...
        # End of code
        cursor_width = (width - borderX) / @column_max
        x = index % @column_max * (cursor_width + @column_spacing)
        y = index / @column_max * @row_height - @virtualOy
        cursor_rect.set(x, y, cursor_width, @row_height)
        refresh if dorefresh || force
    end
end
...
module UpDownArrowMixin
    def initUpDownArrow
        @uparrow   = AnimatedSprite.create("Graphics/Pictures/uparrow", 8, 2, viewport)
        @downarrow = AnimatedSprite.create("Graphics/Pictures/downarrow", 8, 2, viewport)
    ...
    def viewport=(value)
        super
        @uparrow.viewport   = viewport
        @downarrow.viewport = viewport
    end
    ...
    def adjustForZoom(sprite)
        ...
        sprite.x = sprite.x * zoom_x + offset_x / zoom_x
        sprite.y = sprite.y * zoom_y + offset_y / zoom_y
    end

    def update
        super
        @uparrow.x   = x + (width / 2) - (@uparrow.framewidth / 2)
        @downarrow.x = x + (width / 2) - (@downarrow.framewidth / 2)
        @uparrow.y   = y
        @downarrow.y = y + height - @downarrow.frameheight
        @uparrow.visible = visible && active && (top_item != 0 &&
                           @item_max > page_item_max)
        @downarrow.visible = visible && active &&
                             (top_item + page_item_max < @item_max && @item_max > page_item_max)
        @uparrow.z   = z + 1
        @downarrow.z = z + 1
...
class Window_DrawableCommand < SpriteWindow_SelectableEx
    ...
    def getAutoDims(commands, dims, width = nil)
        rowMax = ((commands.length + columns - 1) / columns).to_i
        windowheight = (rowMax * rowHeight)
        windowheight += borderY
        if !width || width < 0
            width = 0
        ...
        dims[0] = [borderX + 1, (width * columns) + borderX +
            (columns - 1) * columnSpacing,].max
        dims[1] = [borderY + 1, windowheight].max
    ...
    def drawCursor(index, rect)
        pbCopyBitmap(contents, @selarrow.bitmap, rect.x, rect.y) if self.index == index
...
class Window_CommandPokemon < Window_DrawableCommand
    ...
    def drawItem(index, _count, rect)
        pbSetSystemFont(contents) if @starting
        rect = drawCursor(index, rect)
        pbDrawShadowText(contents, rect.x, rect.y, rect.width, rect.height,
           _INTL(@commands[index]), baseColor, shadowColor)
    end
...
class Window_AdvancedCommandPokemon < Window_DrawableCommand
    ...
    def textWidth(bitmap, text)
        ...
        chars = getFormattedText(bitmap, 0, 0,
           Graphics.width - borderX - SpriteWindow_Base::TEXTPADDING - 16,
           -1, text, rowHeight, true, true)
        for ch in chars
            dims[0] = dims[0] ? [dims[0], ch[1]].min : ch[1]
            dims[1] = [dims[1], ch[1] + ch[3]].max
        end
        dims[0] = 0 unless dims[0]
    ...
    def drawItem(index, _count, rect)
        ...
        if toUnformattedText(@commands[index]).gsub(/\n/, "") == @commands[index]
            ...
            pbDrawShadowText(contents, rect.x, rect.y, rect.width, rect.height,
               @commands[index], baseColor, shadowColor)
        else
            chars = getFormattedText(
                contents, rect.x, rect.y + 4, rect.width, rect.height,
               @commands[index], rect.height, true, true)
            drawFormattedChars(contents, chars)
        end
    end
end
